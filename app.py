...
# --- 3. SISTEMA DE CONTROL DE SESIÓN ---
...
for i, nombre_pestana in enumerate(nombres_pestanas):
    with tabs[i]:
        ...
        if nombre_pestana == "Códigos SAP":
            columnas_config["hora"] = None
            columnas_config["verificado"] = None
            columnas_config["creado_por"] = None
            columnas_config["cantidad"] = None
                ...
                if archivo_subido is not None:
                    try:
                        ...
                        if registros_nuevos > 0:
                            ...
                            st.session_state.mensaje_toast = "Archivo procesado: Todos los datos ya existían (Cero duplicados agregados)."
                            
                        st.session_state[nombre_clave_sesion] += 1
                        st.rerun()
                        
                    except Exception as e:
                        st.error("🚨 El archivo tiene un formato ilegible o dañado. Verifica que sea un Excel válido (.xlsx).")

        with col_down2:
            st.subheader("📤 Exportar Reporte")
            df_para_exportar = df.drop(columns=['id'])
            if nombre_pestana == "Códigos SAP":
                df_para_exportar = df_para_exportar[['codigo_lamina', 'descripcion']]
    ...
    with tabs[4]: 
        ...
        with col_adm1:
            with st.form("nuevo_usuario", clear_on_submit=True):
                ...
                st.info("💡 Si el usuario ya existe, al guardar se actualizará su contraseña y rol.")
                ...
                if st.form_submit_button("Guardar Usuario"):
                    if n_cedula.strip() != "" and n_password.strip() != "":
                        ...
                        st.error("Debes llenar el usuario y la contraseña.")
