...
# Actualizar la tabla de traslados con las nuevas columnas si no existen
try:
    c.execute("ALTER TABLE traslados ADD COLUMN IF NOT EXISTS despacho INTEGER DEFAULT 0")
    c.execute("ALTER TABLE traslados ADD COLUMN IF NOT EXISTS hora_despacho TEXT")
    c.execute("ALTER TABLE traslados ADD COLUMN IF NOT EXISTS autor_despacho TEXT")
    c.execute("ALTER TABLE traslados ADD COLUMN IF NOT EXISTS pendientes INTEGER DEFAULT 0")
except Exception as e:
    pass # Ignorar si las columnas ya existen
...
def cargar_datos(pestana):
    query = f"""SELECT id, hora, codigo_lamina, descripcion, cantidad, verificado, creado_por, 
                despacho, hora_despacho, autor_despacho, pendientes 
                FROM traslados WHERE pestana='{pestana}'"""
    df = pd.read_sql_query(query, engine)
    
    # Limpiar posibles datos nulos si la tabla es antigua
    if not df.empty and 'despacho' in df.columns:
        df['despacho'] = df['despacho'].fillna(0).astype(int)
        df['pendientes'] = df['pendientes'].fillna(0).astype(int)
        df['hora_despacho'] = df['hora_despacho'].fillna("N/A")
        df['autor_despacho'] = df['autor_despacho'].fillna("N/A")
    return df
...
def guardar_nuevo_registro(pestana, codigo, desc, cant, creador):
    ...
    # Los pendientes iniciales son iguales a la cantidad total si no es SAP
    pendientes_iniciales = cant if pestana != "Códigos SAP" else 0
    c.execute("""INSERT INTO traslados (pestana, hora, codigo_lamina, descripcion, cantidad, verificado, creado_por, despacho, pendientes) 
...
def actualizar_despacho(id_registro, despacho, pendientes, autor):
    hora_actual = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    c.execute("UPDATE traslados SET despacho=%s, pendientes=%s, autor_despacho=%s, hora_despacho=%s WHERE id=%s", 
              (despacho, pendientes, autor, hora_actual, id_registro))

def actualizar_registro(id_registro, codigo, desc, cant):
    # Al modificar la cantidad total, recalculamos los pendientes basándonos en lo ya despachado
    c.execute("SELECT despacho FROM traslados WHERE id=%s", (id_registro,))
    resultado = c.fetchone()
    despacho_actual = int(resultado[0]) if resultado and resultado[0] else 0
    nuevos_pendientes = max(0, cant - despacho_actual)
...
for i, nombre_pestana in enumerate(nombres_pestanas):
    with tabs[i]:
        ...
        # Ocultar botones de agregar/modificar para visualizadores
        if st.session_state.rol in ["administrador", "moderador", "boss"]:
            ...
            with col_exp3:
                with st.expander("🗑️ Eliminar artículos"):
                    if not df_editable.empty:
                        opciones_eliminar = df_editable.apply(lambda r: f"ID: {r['id']} | {r['codigo_lamina']}", axis=1).tolist()
        ...
        permitir_despacho = (st.session_state.rol == "moderador") # SOLO EL MODERADOR EDITA DESPACHOS
        
        # --- CONFIGURACIÓN DE COLUMNAS (PENDIENTES ES AUTOMÁTICO Y DE SOLO LECTURA) ---
        columnas_config = {
            "id": None, 
            ...
            "pendientes": st.column_config.NumberColumn("⏳ Pendientes", disabled=True), # Bloqueado
            "hora_despacho": st.column_config.TextColumn("🕒 Hora Desp.", disabled=True),
            "autor_despacho": st.column_config.TextColumn("👷 Autor Desp.", disabled=True),
        }
        
        if nombre_pestana == "Códigos SAP":
            for col in ["hora", "verificado", "creado_por", "cantidad", "despacho", "pendientes", "hora_despacho", "autor_despacho"]:
        ...
        if not df.empty and nombre_pestana != "Códigos SAP":
            ...
            for index, row in edited_df.iterrows():
                # 1. Si el administrador o boss verificó
                if permitir_verificacion and row['verificado'] != df.loc[index, 'verificado']:
                    actualizar_verificacion(row['id'], row['verificado'])
                    cambios_realizados = True
                
                # 2. Si el moderador editó el despacho
                if permitir_despacho and row['despacho'] != df.loc[index, 'despacho']:
                    cant_total = int(row['cantidad'])
                    cant_despacho = int(row['despacho'])
                    
                    # CÁLCULO AUTOMÁTICO DE PENDIENTES
                    calc_pendientes = cant_total - cant_despacho
                    if calc_pendientes < 0:
                        calc_pendientes = 0 
                    
                    actualizar_despacho(row['id'], cant_despacho, calc_pendientes, st.session_state.usuario)
                    cambios_realizados = True
